...
export const useProjectStore = defineStore('project', () => {
  ...
  function clearProject() {
    ...
    // 重置视图设置
    viewSettings.value = { iconScale: 100, labelColor: '#000000', mapOpacity: 1.0 }
  }
  ...
  function loadProject(projectData: IProject) {
    ...
    if (projectData.viewSettings) {
      viewSettings.value = projectData.viewSettings
    }
  ...
  async function saveToDisk() {
    const data: IProject = { 
      ...
      viewSettings: viewSettings.value, // [保存] 视图设置
  ...
  return {
    ...
    viewSettings, // 导出 state
    updateViewSettings, // 导出 action
