...
import type { IProject, INode, IEdge, IBuilding, ILoop, IViewSettings, ICamera2D, ICamera3D } from '../types'
...
export const useProjectStore = defineStore('project', () => {
  ...
  function addLoop(name: string, rawNodes: any[], processedEdges: IEdge[], fileName: string) {
    ...
    const newLoop: ILoop = { id: newLoopId, name: name, htmlSource: fileName, deviceCount: rawNodes.length }
    const newNodes = processRawNodes(rawNodes, newLoopId, 'normal')
    loops.value.push(newLoop); nodes.value.push(...newNodes); edges.value.push(...processedEdges)
    structureVersion.value++; ElMessage.success(`成功添加 ${name}`)
  }
  ...
  function deleteLoop(loopId: string) {
    ...
    structureVersion.value++; ElMessage.success('Loop 已删除')
  }
  ...
  function replaceLoop(loop: ILoop, rawNodes: any[], rawEdges: any[], fileName: string) {
    ...
    nodes.value.push(...newNodes); const newEdges = processRawEdges(rawEdges, newNodes); edges.value.push(...newEdges); cleanInvalidEdges()
    const targetLoop = loops.value.find(l => l.id === loop.id)
    if (targetLoop) { targetLoop.htmlSource = fileName; targetLoop.deviceCount = newNodes.length }
    structureVersion.value++; ElMessage.success(`Loop 替换成功`)
  }

  function updateLoop(loop: ILoop, rawNodes: any[], rawEdges: any[], fileName: string) {
    const loopId = loop.id; const existingNodes = nodes.value.filter(n => n.loopId === loopId); const otherNodes = nodes.value.filter(n => n.loopId !== loopId)
    const oldMap = new Map<string, INode>(); existingNodes.forEach(n => oldMap.set(n.mac, n))
    const processedNewNodes: INode[] = []; const newMacs = new Set<string>()
    rawNodes.forEach(raw => {
      const mac = extractMac(raw); newMacs.add(mac); const oldNode = oldMap.get(mac)
      if (oldNode) { oldNode.shortId = raw.rloc16 || ''; oldNode.role = parseRole(raw.role); oldNode.diffStatus = 'normal'; processedNewNodes.push(oldNode) }
      else { const newNode = createNodeFromRaw(raw, loopId); newNode.diffStatus = 'new'; newNode.isPlaced = false; newNode.position = null; processedNewNodes.push(newNode) }
    })
    const missingNodes: INode[] = []; existingNodes.forEach(old => { if (!newMacs.has(old.mac)) { old.diffStatus = 'missing'; missingNodes.push(old) } })
    ...
    const newEdges = processRawEdges(rawEdges, processedNewNodes); edges.value.push(...newEdges)
    const targetLoop = loops.value.find(l => l.id === loopId)
    if (targetLoop) { targetLoop.htmlSource = fileName; targetLoop.deviceCount = processedNewNodes.length + missingNodes.length }
    const newCount = processedNewNodes.filter(n => n.diffStatus === 'new').length; const missingCount = missingNodes.length
    structureVersion.value++; ElMessage.success(`Loop 更新完成：新增 ${newCount}，缺失 ${missingCount}`)
  }

  function purgeMissingNodes(loopId: string) {
    const initialCount = nodes.value.length; nodes.value = nodes.value.filter(n => !(n.loopId === loopId && n.diffStatus === 'missing'))
    const deletedCount = initialCount - nodes.value.length; const targetLoop = loops.value.find(l => l.id === loopId)
    if (targetLoop) targetLoop.deviceCount = nodes.value.filter(n => n.loopId === loopId).length
    structureVersion.value++; ElMessage.success(`已清理 ${deletedCount} 个缺失设备`)
  }

  function confirmLoopChanges(loopId: string) {
    let count = 0; nodes.value.forEach(n => { if (n.loopId === loopId && n.diffStatus === 'new') { n.diffStatus = 'normal'; count++ } })
    ElMessage.success(`已确认 ${count} 个新增设备`)
  }

  function deleteNode(nodeId: string) {
    nodes.value = nodes.value.filter(n => n.id !== nodeId); cleanInvalidEdges(); selectedNodeId.value = null; structureVersion.value++ 
  }

  function createNodeFromRaw(raw: any, loopId: string): INode {
    const mac = extractMac(raw); const defaultBld = buildings.value.length > 0 ? buildings.value[0].id : '1'; const defaultFlr = buildings.value.length > 0 && buildings.value[0].floors.length > 0 ? buildings.value[0].floors[0].id : '1'
    return { id: mac, mac: mac, shortId: raw.rloc16 || '', role: parseRole(raw.role), type: raw.deviceType || DeviceType.DEFAULT, label: raw.label || mac.slice(-4), buildingId: defaultBld, floorId: defaultFlr, position: null, isPlaced: false, loopId: loopId, diffStatus: 'normal' }
  }
  function processRawNodes(rawNodes: any[], loopId: string, status: 'normal' | 'new') { return rawNodes.map(raw => { const node = createNodeFromRaw(raw, loopId); node.diffStatus = status; return node }) }
  function processRawEdges(rawEdges: any[], validNodes: INode[]) { return rawEdges as IEdge[] }
  function parseRole(roleStr: string): DeviceRole { const r = (roleStr || '').toLowerCase(); if (r.includes('leader')) return DeviceRole.LEADER; if (r.includes('router')) return DeviceRole.ROUTER; return DeviceRole.REED }
  function cleanInvalidEdges() { const validNodeIds = new Set(nodes.value.map(n => n.id)); edges.value = edges.value.filter(e => validNodeIds.has(e.sourceId) && validNodeIds.has(e.targetId)) }
